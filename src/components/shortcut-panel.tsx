...
export default function ShortcutPanel() {
  ...
  return (
    <div className="-z-20 w-[min(560px,95%)] relative mt-1">
...
function ShortcutPanelItem({ open, _id, name, url, icon }: { open: (x: string, props: any) => void } & ShortcutType) {
  ...
  return (
    <div className="flex flex-col gap-y-5 justify-center items-center p-3 rounded-lg hover:bg-cl/10 backdrop-blur size-28 transition-all pointer select-none relative [&:hover_.shortcut-item-edit-menu]:opacity-20" onClick={(e: any) => {
      ...
      <div className="size-1/2 bg-cl/5 rounded-full overflow-hidden">
        <img src={icon} alt={name} className="size-full p-1" />
      </div>
...
function ShortcutPanelAddItem({ open }: { open: (x: string, props: any) => void }) {
  const dg = useDialog();
  return (
    <div className="flex flex-col justify-between items-center p-4 rounded-lg hover:bg-cl/10 size-28 transition-all pointer select-none [&:hover_.shortcut-item-edit-menu]:opacity-50 active:scale-95" onClick={() => open('shortcuts', { action: "add" })}>
      <Add className='p-1.5 scale-[2] bg-bd/20 rounded-full translate-y-3' />
